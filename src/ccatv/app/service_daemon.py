...
def run_service_daemon(
    ...
    *,
    ...
    # Initialize last_daily_sync_date to prevent running sync immediately on startup.
    # This ensures adapters won't be blocked when the service restarts, regardless of time of day.
    # Sync will run at the next scheduled sync_time (e.g., 03:00 tomorrow if restarting today).
    last_daily_sync_date: str | None = None
    if enable_daily_metadata_sync:
        now_local = datetime.now().astimezone()
        # Always initialize to today; sync will run at next sync_time, never immediately on startup
