...
def test_initialize_database_creates_expected_tables(tmp_path: Path) -> None:
    ...
    assert "serviceinfo_cache" in tables
    assert "films_ignore_rules" in tables
    assert "auto_record_titles" in tables
...
def test_migration_v12_adds_is_radio_channel_column(tmp_path: Path) -> None:
    db_path = tmp_path / "ccatv.sqlite3"
    connection = initialize_database(db_path)
    try:
        columns = connection.execute("PRAGMA table_info(epg_channels)").fetchall()
